...
//        holder.bind(data,  position, getItemCount)
        with(holder.binding) {
            invoice.text = holder.itemView.resources.getString(com.project.fishbud.R.string.invoice, data.idPesanan)
            produkName.text = data.namaIkan
            total.text =
                holder.itemView.resources.getString(com.project.fishbud.R.string.total_new_order, formatRupiah(data.harga, 0))
            val tmp = (data.harga * 0.05).toLong()
            adminCost.text = "Biaya Admin (5%) : ${formatRupiah(tmp, 0)}"
            pendapatan.text =
                holder.itemView.resources.getString(com.project.fishbud.R.string.pendapatan, formatRupiah((data.harga - tmp), 0))

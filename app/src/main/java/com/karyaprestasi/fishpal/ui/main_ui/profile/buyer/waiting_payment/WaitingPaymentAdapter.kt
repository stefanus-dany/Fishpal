...
import com.karyaprestasi.fishpal.databinding.ItemWaitingPaymentBinding
...
//        holder.bind(data, position, dataPembayaran.size)
        with(holder.binding) {
            dateOrder.text = holder.itemView.resources.getString(R.string.date_order, data.date)
            //date + 1 day
            ...
            payBefore.text = holder.itemView.resources.getString(R.string.pay_before, dt)
            total.text = holder.itemView.resources.getString(
                R.string.total_waiting_payment,
                formatRupiah(data.totalHarga)
            )
            idPayment.text =
                holder.itemView.resources.getString(R.string.id_payment, data.idPembayaran)
            idCreditCard.text =
                holder.itemView.resources.getString(R.string.no_credit_card, data.kartuKredit)
